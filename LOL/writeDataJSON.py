...
def writeChampionsJSON() :
    with open('../LOL/champion_data/champion.json', 'w',encoding='UTF-8-sig') as convert_file:
...
def writeChampionJSON(champion_kor_name) :
    ...
    with open(champion_json_file_name, 'w',encoding='UTF-8-sig') as convert_file:
        json.dump(champion(champion_kor_name), convert_file, ensure_ascii = False)

#LOL 폴더에 아이템 정보 JSON 파일 생성
def writeItemJSON() :
    with open('../LOL/champion_data/item.json', 'w',encoding='UTF-8-sig') as convert_file:
